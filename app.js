...
async function loadPeopleData() {
  const response = await fetch('assets/data/people.json?v=83', { cache: 'no-cache' });
...
function splitDescription(person) {
  ...
  if (!raw) return {facts: [], life: [], event: [], memory: [], allParagraphs: []};

  const personalHeading = /^(פרטים אישיים|פרטים אישיים והנצחה)$/;
  const lifeHeading = /^(סיפור חיים|סיפור חייו|סיפור חייה|קורות חיים|קורות חיים ונפילה|דרך, עשייה ואהבות|שירות צבאי וחיים אחרי השחרור|שירות צבאי|חיים אחרי השחרור|ילדות ונעורים|לימודים ושירות צבאי|עשייה ואהבות)$/;
  ...
  const memoryHeading = /^(דמותו וזכרו|דמותה וזכרה|דמות וזיכרון|דברי זיכרון|זיכרון ומילים מהלב|על מצבתו נכתב|על מצבתה נכתב|מילים שנחקקו|הנצחה|זיכרון)$/;
  const isFactLine = line => /^(בן|בת)\s+\d|^בן\s+|^בת\s+|^אח\s+|^אחות\s+|^אב\s+|^אם\s+|^בעלה\s+|^אשתו\s+|^בן זוג|^בת זוג|^נולד|^נולדה|^תאריך|^נרצח|^נרצחה|^נפל|^נפלה|^נהרג|^נהרגה|^נחטף|^נחטפה|^הושב|^הושבה|^מקום מגורים|^התגורר|^התגוררה|^גדל|^גדלה|^מקום מנוחה|^מקום קבורה|^תפקיד|^שירת|^סרן|^רס״ן|^רס"ן|^רב-סמל|^חבר כיתת|^סגן רבש|^משפחה:|^קרבה משפחתית:|^שאירים|^הובא|^הובאה|^חלקה|^שורה|^קבר|^גוש|^אזור|^.+מונצח|^.+מונצחת/.test(line);
  ...
  for (const rawLine of rawLines) {
    ...
    if (!rawTrim) continue;
    const headingLine = rawTrim.replace(/^פרטים אישיים והנצחה\s*$/, 'פרטים אישיים').trim();

    // Important: detect section headings before cleanFactLine(), because that
    // helper intentionally removes headings like "סיפור חיים" from display.
    ...
    if (section === 'facts') {
      // When the text has a real "פרטים אישיים" section, keep every line there
      // until the next section heading. This preserves lines like "נחטף..." and
      // "נהרג..." as personal details instead of mixing them into the story.
  ...
  // Fallback for older texts that have no section headings: classify the body
  // by content, but only after the opening fact block was extracted.
  if (!knownSectionParagraphs.length) {
    const bodyLines = rawLines.map(l => cleanFactLine(l.trim())).filter(Boolean).filter(line => !facts.includes(line) && !personalHeading.test(line));
    for (const p of bodyLines) {
      if (lifeHeading.test(p) || eventHeading.test(p) || memoryHeading.test(p)) continue;
      if (/כתבו|ספדו|נזכור|יזכרו|חבריו|חברותיה|משפחת|אשתו|אמו|אביו|אחיו|אחותו|בתו|בנו|כתב:|כתבה:|אמר|סיפר|סיפרה|על מצבת/.test(p)) addUnique(memory, p);
