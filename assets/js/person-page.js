...
`;
  ...
  const renderTopMedia = () => (person.topMedia || []).map((group, groupIndex) => {
    ...
    return `<section class="media-section" aria-labelledby="mediaHeading${groupIndex}"><h2 id="mediaHeading${groupIndex}">${esc(group.heading || 'סרטון לזכרו')}</h2>${vids ? `<div class="media-grid">${vids}</div>` : ''}${links ? `<div class="media-actions">${links}</div>` : ''}</section>`;
  }).join('');
