...
function pic(p,cls=''){const pos=/^\d{1,3}(?:\.\d+)?%\s+\d{1,3}(?:\.\d+)?%$/.test(String(p.portrait?.position||''))?p.portrait.position:'50% 38%';const fit=p.portrait?.fit==='contain'?'contain':'cover';const rawScale=Number(p.portrait?.scale);const scale=Number.isFinite(rawScale)&&rawScale>=1&&rawScale<=3.75?rawScale:1;const rawPad=Number(p.portrait?.pad);const pad=Number.isFinite(rawPad)&&rawPad>=0&&rawPad<=22?rawPad:0;const rawTx=Number(p.portrait?.tx),rawTy=Number(p.portrait?.ty);const tx=Number.isFinite(rawTx)&&Math.abs(rawTx)<=140?rawTx:0,ty=Number.isFinite(rawTy)&&Math.abs(rawTy)<=140?rawTy:0;const bg=/^#[0-9a-f]{6}$/i.test(String(p.portrait?.bg||''))?p.portrait.bg:'#d7dde0';const src=String(p.image||fallbackPortrait);const altText=p.image?displayName(p):('נר זיכרון לזכר '+displayName(p));return `<img class="${cls}" src="${e(src)}" alt="${e(altText)}" loading="lazy" decoding="async" style="--fit:${fit};--pos:${e(pos)};--scale:${scale};padding:${pad}%;background:${e(bg)};transform:translate(${tx}%,${ty}%) scale(${scale})">`}
...
function openPreview(p,hash=true){active=p;lastFocus=document.activeElement;if(hash)setHash(p);boxTitle.textContent=displayName(p);boxPlace.textContent=p.place||'';boxRole.textContent=p.role||'';boxRole.hidden=!String(p.role||'').trim();boxService.innerHTML=service(p);boxFacts.innerHTML=(p.generalDetails||[]).map(x=>`<li>${e(x)}</li>`).join('');boxImg.innerHTML=pic(p);const first=firstName(p);pageLink.href=p.page;pageLink.textContent=`לדף של ${first}`;pageLink.setAttribute('aria-label',`לדף של ${first}`);pageLink.hidden=false;pageLink.removeAttribute('hidden');copyBtn.textContent='העתקת קישור';box.inert=false;box.removeAttribute('inert');box.setAttribute('aria-hidden','false');box.classList.add('is-open');document.body.style.overflow='hidden';setTimeout(()=>closeBtn.focus({preventScroll:true}),0)}
